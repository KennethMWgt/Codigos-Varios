...
import sqlite3
import dbUtils
from dbUtils import create_tables, insert_task, update_task, delete_task, get_all_tasks
...
class SQLiteTasksTestCase(unittest.TestCase):
    def setUp(self):
        ...
        # 🔹 Sobrescribir conexión global en dbUtils
        dbUtils.override_conn_for_tests(self.conn)

        create_tables()

    def tearDown(self):
        # Restaurar a None (para no afectar otros tests)
        dbUtils.override_conn_for_tests(None)
        self.conn.close()
    ...
    def test_create_tables_is_idempotent(self):
        create_tables() 
        tid = insert_task("First task")  
        self.assertIsInstance(tid, int)

    def test_insert_and_readback(self):
        tid = insert_task("Buy milk")
        rows = self.fetch_all()
    ...
    def test_update_title_and_done(self):
        tid = insert_task("Initial title")  

        count = update_task(tid, title="Updated title")
        self.assertEqual(count, 1)
        row = self.fetch_all()[0]
        self.assertEqual(row["title"], "Updated title")
        self.assertEqual(row["is_done"], 0)

        count = update_task(tid, done=1)
        self.assertEqual(count, 1)
        row = self.fetch_all()[0]
        self.assertEqual(row["is_done"], 1)

        count = update_task(tid, title="Final title", done=0)
        self.assertEqual(count, 1)
        row = self.fetch_all()[0]
    ...
    def test_update_with_no_fields_returns_zero(self):
        tid = insert_task("No-op")
        count = update_task(tid)  
    ...
    def test_delete_existing_and_nonexistent(self):
        tid = insert_task("To delete")
        count = delete_task(tid)
        self.assertEqual(count, 1)
